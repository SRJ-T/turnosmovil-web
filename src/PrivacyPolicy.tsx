...
function Logo() {
  return (
    <Link to="/" className="flex items-center gap-2.5 no-underline">
      ...
      <span style={{ color:'#fff', fontWeight:800, fontSize:27, letterSpacing:'-0.3px' }}>Turnos Móvil</span>
    </Link>
  );
}
    ...
    <div style={{ background: B.bg, minHeight: '100vh', color: B.white, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <nav style={{ borderBottom: `1px solid ${B.border}`, padding: '0 24px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Logo />
          <Link to="/" style={{ color: B.muted, fontSize: 21, textDecoration: 'none' }}>← Volver al inicio</Link>
        </div>
      </nav>
      ...
      <main style={{ maxWidth: 800, margin: '0 auto', padding: '48px 24px 80px' }}>
        <div style={{ marginBottom: 40 }}>
          <p style={{ color: B.blue, fontSize: 18, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 12 }}>Legal</p>
          <h1 style={{ fontSize: 54, fontWeight: 800, margin: '0 0 12px', lineHeight: 1.2 }}>Política de Privacidad</h1>
          <p style={{ color: B.muted, fontSize: 21 }}>Última actualización: 20 de agosto de 2026</p>
        </div>

        <div style={{ color: B.muted, fontSize: 23, lineHeight: 1.8 }}>
      ...
      <footer style={{ borderTop: `1px solid ${B.border}`, padding: '24px', textAlign: 'center' }}>
        <p style={{ color: B.dim, fontSize: 20 }}>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 36 }}>
      <h2 style={{ color: '#FFFFFF', fontSize: 27, fontWeight: 700, marginBottom: 12, marginTop: 0 }}>{title}</h2>
      {children}
    </div>
  );
}
